...
{
    public class EightBoxBody : RigidBody3D
    {
        public EightBoxBody()
            : base(Vector3.Zero, Vector3.Zero, 0f, 0f, 0f)
        {
        ...
        public override GraphicsToolkit.Physics._3D.Contact3D GenerateContact(RigidBody3D rb, float dt)
        {
            if (rb as SphereBody != null)
            {
                ...
                pa.X = MathHelper.Clamp(pa.X, -0.5f, 0.0f);
                pa.Y = MathHelper.Clamp(pa.Y, -0.5f, 0.0f);
                pa.Z = MathHelper.Clamp(pa.Z, -0.5f, 0.0f);
