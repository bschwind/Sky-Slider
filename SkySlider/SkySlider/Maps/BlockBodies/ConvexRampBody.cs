...
{
    public class ConvexRampBody : RigidBody3D
    {
        public ConvexRampBody()
        ...
        public override GraphicsToolkit.Physics._3D.Contact3D GenerateContact(RigidBody3D rb, float dt)
        {
            if (rb as SphereBody != null)
            {
                ...
                if (pa.X < -0.5)
                {
                    pa.X = -0.5f; //Clamp minimum x
                }
                if (pa.Y < -0.5)
                {
                    pa.Y = -0.5f; //Clamp minimum y
                }
                Vector2 XYVec = new Vector2(pa.X + 0.5f, pa.Y + 0.5f);
                if (XYVec.LengthSquared() > 1) //if outside block up or to the right
                {
